...
display_info() {
    ...
    if ! [ $INFO_CODE -eq $ABORTED_BY_USER ]
        then printf printf "Wavpack file testing tool V: ${WAVPACK_TEST_V}\n"
    fi
    case $INFO_CODE in
        $WAVPACK_ERROR )
...
##################################################
# Constants
WAVPACK_TEST_V="09092024"   # Tool version
...
# Info codes
WAVPACK_ERROR=1     # Wavpack not installed
...
if ! [ -x "$(command -v wvunpack)" ]
    then display_info $WAVPACK_ERROR
         exit 1
fi
...
do
    ...
    taskset -c ${CORENUM} wvunpack -v "$LINE" > $TEMPFILE 2>&1
    if [ $? -eq "0" ]
